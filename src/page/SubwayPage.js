...
function SubwayPage({ theme, lang }) {
  ...
  const Nextreceive = () => {
    // 도착 예정시간 받아오는거
    axiosInstance
      .get(`/subway/arrival?subwayNm=${searchParams.get('subwayNm')}&updnLine=${trainfo.updnLine}&statnNm=${trainfo.statnNm}`)
      .then((res) => {
              ...
              {nowtrain &&
                (trainfo.subwayHeading == 0 && (trainfo.trainSttus == 0 || trainfo.trainSttus == 1) ? (
                  trainfo?.statnNm?.length > 5 ? (
                    ...
                    <div style={{ position: 'absolute', fontSize: '40px', bottom: 13, left: -40, opacity: '100%', color: theme.gray1 }}>
                      <RiArrowLeftSLine />
                    </div>
                  )
                ) : trainfo?.statnNm?.length > 5 ? (
                  ...
                  <div style={{ position: 'absolute', fontSize: '40px', top: 13, left: -40, opacity: '20%', color: theme.gray1 }}>
                    <RiArrowLeftSLine />
                  </div>
                ))}
